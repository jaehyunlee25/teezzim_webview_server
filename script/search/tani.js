...
function mneCallDetail(arrDate) {
	...
	post('ReservationTimeList.asp', param, {}, data => {
        ...
        Array.from(trs).forEach(tr => {
            ...
            if(!course) return;
            const time = tr.children[2].innerText.replace(/\s/g,"").gh(5).split(":").join("");
            const fee_discount = tr.children[3].innerText.replace(/\s/g,"").ct(4).split(',').join('') * 1;
            const fee_normal = tr.children[3].innerText.replace(/\s/g,"").ct(4).split(',').join('') * 1;
